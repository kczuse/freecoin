...
QString QRCodeDialog::getURI()
{
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non FEC unit input in lnReqAmount, we generate the URI with FEC as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(FreecoinUnits::format(FreecoinUnits::FEC, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
